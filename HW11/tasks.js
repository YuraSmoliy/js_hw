...
function calcArrProduct(arr) {
  return new Promise(function (resolve) {
    ...
    if (numbers.length !== arr.length) resolve("Error! Incorrect array!");
    resolve(numbers.reduce((previous, current) => previous * current));
  });
}
...
async function showNumbers() {
  let i = 0;
  while (i <= 10) {
    console.log(await delay(i, Math.floor(Math.random() * 3000)));
    i++;
  }
}
showNumbers();
