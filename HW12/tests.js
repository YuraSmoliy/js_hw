...
const userDataPath = './userData.txt';
const fs = require('fs');
function writeUserData(text){
  fs.appendFileSync(userDataPath, text + '\n');
  // fs.writeFile(userDataPath, text, (err) => {if (err) throw err;});
}
...
http.createServer( function(request, response) {
  ...
  if (query) {
    const text = decodeURIComponent(query.split('=')[1]);
    writeUserData(text);
  }
  const fileContent = fs.readFileSync(userDataPath, 'utf8')
  response.writeHead(200, {'Content-Type': 'text/html'});
  response.write(
    `<div>Yor message ${fileContent}</div>`
  );
  response.end();
}).listen(5000);
